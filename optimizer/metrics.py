...
def hausdorff_distance(
    ...
    dists_orig_to_simp = np.asarray(pcd_orig.compute_point_cloud_distance(pcd_simp))
    dists_simp_to_orig = np.asarray(pcd_simp.compute_point_cloud_distance(pcd_orig))
...
def build_report(
    ...
    out_size = (
        Path(output_path).stat().st_size / (1024 * 1024) if Path(output_path).exists() else 0.0
    )
...
def print_report(report: dict) -> None:
    ...
    print(
        f"  Triangles     : {report['original_triangles']:,}  →  {report['reduced_triangles']:,}"
    )
    print(f"  Reduction     : {report['reduction_percent']}%")
    before, after = report["file_size_before_mb"], report["file_size_after_mb"]
    print(f"  File size     : {before} MB  →  {after} MB")
