...
def export_urdf_pair(
    ...
    log.info("Generating visual mesh (%s triangles) …", f"{visual_triangles:,}")
    visual_mesh, vis_time = simplify_mesh(mesh, visual_triangles)
    save_mesh(visual_mesh, visual_path)

    log.info("Generating collision mesh (%s triangles) …", f"{collision_triangles:,}")
...
def _write_urdf_snippet(
  ...
  <collision>
    ...
    log.info("URDF snippet saved → %s", snippet_path)
